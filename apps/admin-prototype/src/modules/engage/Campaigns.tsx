...
import { ModuleHeader, ErrorPanel, Screen, percent, useModuleData, useUserName } from './parts'
...
/**
 * Eighteen columns made this the widest table in the app - a 2,200px scroll
 * before anybody had asked a question. The eight shown by default answer "what
 * is this campaign, and did it produce anything"; the ten funnel and
 * configuration columns are one click away and stay in the URL, so a curated
 * view is still a link. Revenue attributed is a default and open rate is not,
 * which is the PRD's own ordering.
 */
const COLUMN_CATALOGUE: ColumnCatalogueEntry[] = [
  ...
  { key: 'audience', label: 'Audience size', defaultVisible: true },
  ...
  { key: 'utm', label: 'UTM set', defaultVisible: false },
  { key: 'owner', label: 'Owner', defaultVisible: false },
]

export default function EngageCampaigns() {
  ...
  const open = openId ? campaigns.find((c) => c.id === openId) : undefined
  ...
  const allColumns: Record<string, Column<Campaign>> = Object.fromEntries(
    ...
    },
    {
      ...
      cell: (c) =>
        c.budget === null ? (
          <span className="text-text-secondary">No budget</span>
        ) : (
          <MoneyCell kobo={c.budget} tone="muted" />
        ),
      width: 130,
    },
    {
    ...
    },
    {
      ...
      cell: (c) => <MoneyCell kobo={c.stats.revenueAttributed} tone={c.stats.revenueAttributed > 0 ? 'positive' : 'muted'} strong />,
      width: 170,
    },
    {
      key: 'utm',
    ...
    ] as Array<Column<Campaign>>).map((column) => [column.key, column]),
  )
  ...
  return (
    <Screen>
      <ModuleHeader
        title="Campaigns"
        description="Every campaign carries its attributed enrolments and revenue, not only its open rate."
        actions={
          ...
          <DataTable
            ...
            emptyMessage={
              search || Object.values(filters).some(Boolean)
                ...
                : 'A campaign sends one template to one segment. Nothing is attributed until one runs.'
            }
            emptyAction={
              search || Object.values(filters).some(Boolean) ? undefined : (
          ...
          open ? (
            <div className="flex w-full flex-wrap items-center justify-between gap-3">
              <p className="text-body-12 text-text-secondary">
                A campaign that has already sent is not edited - its figures describe what really happened.
              </p>
              <Button
                size="sm"
                variant="secondary"
      ...
      >
        {open && (
          <div className="space-y-6">
            {open.status === 'draft' && (
              <Alert tone="info" title="This campaign has not sent yet">
                Everything about it is still editable in the builder - audience, template and schedule included.
              </Alert>
            )}

            <Card padding="none">
              <CardHeader title="Setup" description="Edited in the builder, so a sent campaign cannot drift from what went out." />
              <CardBody>
                <KeyValueList columns={2}>
                  ...
                  <KeyValue label="Segment">
                    <Link to="/engage/segments" className="text-accent hover:underline">
                      {segmentName(open.segmentId as string)}
                    </Link>
                  </KeyValue>
                  <KeyValue label="Audience size">{formatNumber(open.audienceSize)}</KeyValue>
                  <KeyValue label="Template">
                    <Link to="/engage/templates" className="text-accent hover:underline">
                      {templateName(open.templateId as string)}
                    </Link>
                  </KeyValue>
                  <KeyValue label="Scheduled">
                    {open.scheduledAt ? formatDateTime(open.scheduledAt) : 'Not scheduled'}
                  </KeyValue>
                  ...
                  <KeyValue label="Budget">
                    {open.budget === null ? 'No budget' : formatNaira(open.budget)}
                  </KeyValue>
                  <KeyValue label="UTM">
            ...
            <Card padding="none">
              ...
              <CardBody className="space-y-3">
                ...
                <div className="mt-4 rounded-xl border border-success-line bg-success-fill px-4 py-3">
                  ...
                  <p className="mt-1 text-heading-24 text-success-ink tabular-nums">
                    {formatNaira(open.stats.revenueAttributed)}
                  </p>
              ...
              <CardBody>
                <p className="text-body-14 text-text-secondary">
                  ...
                  {open.stats.delivered > 0
                    ...
                    : '.'}{' '}
                  An unsubscribe is held on the Person record, so it applies across every channel and every
                  future campaign without anyone copying a list.
                </p>
              </CardBody>
            </Card>
          </div>
